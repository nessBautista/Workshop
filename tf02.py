#Implementation of a simple convolutional neural network using MNIST-dataset
...
plotFunctions.plot_images(images=images, cls_true=cls_true, img_shape= img_shape)

#----------------------------------------------CONVOLUTIONAL NEURAL NETWORK
#Remember the main components of the design:
#   - The placeholders
#   - The variables (wights and biases)
...
y_true_cls = tf.argmax(y_true, dimension=1) #Extract the highest probability
...
model_builder = tf.saved_model.builder.SavedModelBuilder("exported_model_mnist")
inputs = {
        'inputs': tf.saved_model.utils.build_tensor_info(x)
}
outputs = {
        'output': tf.saved_model.utils.build_tensor_info(y_pred_cls)
}
